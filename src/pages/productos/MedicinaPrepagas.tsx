import { Layout } from "@/components/layout/Layout";
import { Button } from "@/components/ui/button-variants";
import { CheckCircle2, MessageCircle } from "lucide-react";
import { motion } from "framer-motion";
import { useParams } from "react-router-dom";
import { buildWhatsAppUrl } from "@/utils/whatsapp";

export default function MedicinaPrepagas() {
  const { provider } = useParams();
  const providerName = provider?.split("-").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ") || "Medicina Prepaga";
  const waUrl = buildWhatsAppUrl(`un plan de Medicina Prepaga ${providerName}`);

  return (
    <Layout>
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-white py-24">
        <div className="container mx-auto px-4 max-w-5xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16">
            <h1 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-4">{providerName}</h1>
            <p className="text-xl text-gray-600 max-w-2xl mx-auto">
              Encontrá el plan de salud que se adapta a vos y tu familia
            </p>
          </motion.div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
              <h2 className="text-2xl font-bold text-gray-900 mb-6">Beneficios</h2>
              <div className="space-y-4">
                {[
                  ["Red de prestadores premium", "Acceso a los mejores profesionales y centros médicos"],
                  ["Cobertura integral", "Incluye consultas, estudios, internación y cirugías"],
                  ["Sin límite de edad", "Planes disponibles para todas las edades"],
                  ["Emergencias 24/7", "Cobertura de urgencias las 24 horas del día"],
                ].map(([t, d]) => (
                  <div key={t} className="flex items-start gap-3">
                    <CheckCircle2 className="w-6 h-6 text-primary-600 flex-shrink-0 mt-1" />
                    <div>
                      <h3 className="font-semibold text-gray-900">{t}</h3>
                      <p className="text-gray-600">{d}</p>
                    </div>
                  </div>
                ))}
              </div>
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}>
              <h2 className="text-2xl font-bold text-gray-900 mb-6">Coberturas Incluidas</h2>
              <div className="bg-white rounded-xl p-6 shadow-md">
                <ul className="space-y-3">
                  {["Medicina clínica y especialidades","Estudios de diagnóstico","Internación en habitación compartida/privada","Cirugías programadas y de urgencia","Maternidad y pediatría","Odontología básica"].map(c => (
                    <li key={c} className="flex items-center gap-2">
                      <CheckCircle2 className="w-5 h-5 text-green-600" />
                      <span className="text-gray-700">{c}</span>
                    </li>
                  ))}
                </ul>
              </div>
            </motion.div>
          </div>

          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.4 }}
            className="bg-gradient-to-br from-primary-600 to-primary-700 rounded-2xl shadow-xl p-8 lg:p-12 text-center text-white"
          >
            <h2 className="text-3xl font-bold mb-4">Consultanos por WhatsApp</h2>
            <p className="text-xl mb-8 text-white/90">
              Te ayudamos a elegir tu cobertura. Te respondemos en el momento. Atención: L a V de 9 a 18 h.
            </p>
            <Button variant="hero" size="lg" asChild>
              <a href={waUrl} target="_blank" rel="noopener noreferrer">
                <MessageCircle className="w-5 h-5" />
                Hablar por WhatsApp
              </a>
            </Button>
          </motion.div>
        </div>
      </div>
    </Layout>
  );
}
